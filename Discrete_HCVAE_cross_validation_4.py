...
class MatrixSetLoader(object):
    ...
    def dataloader(self):
        ...
        for i in range(int(4*n_valid), len(datasets_expert_matrix)):
            expert_matrix_train[i-int(3*n_valid)] = datasets_expert_matrix[i]
        for j in range(int(3*n_valid)):
            expert_matrix_train[j] = datasets_expert_matrix[j]
        for k in range(int(3*n_valid), int(4*n_valid)):
            expert_matrix_valid[k - int(3*n_valid)] = datasets_expert_matrix[k]
...
def valid(model, valid_rawmatrix, matrix_order):
    ...
    for i in range(0, matrix_number):
        ...
        file_name_1 = savefile_ci + 'original matrix CI_4.csv'
        with open(file_name_1, "a") as df_1:
            df_1.write(str(temp_x_ci))
            df_1.write("\n")
        file_name_2 = savefile_ci + 'Discrete HCVAE generating matrix CI_4.csv'
...
def main():
    ...
    cross_valid.consistent_comparison_results_obtained(re_consistent_matrix_iter, dataset_valid, "cross_4")
